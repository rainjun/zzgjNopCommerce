...
namespace Nop.Services.Customers
{
    ...
    public partial class CustomerRegistrationService : ICustomerRegistrationService
    {
        ...
        public virtual CustomerLoginResults ValidateCustomer(string usernameOrEmail, string password)
        {
            ...
            if (!customer.IsRegistered() && !customer.IsInCustomerRole("Distributors", false))
        ...
        public virtual CustomerRegistrationResult RegisterCustomer(CustomerRegistrationRequest request)
        {
            ...
            //rainAdd checkDateOfBirth
            DateTime? DateOfBirth = DateTime.UtcNow;
            if (!string.IsNullOrWhiteSpace(request.IdentityId) && !CommonHelper.IsValidIdCardNo(request.IdentityId, out DateOfBirth))
            {
                result.AddError(_localizationService.GetResource("Common.WrongIdentityId"));
                return result;
            }
            ...
            //rainAdd 增加身份证
            request.Customer.IdentityId = request.IdentityId;
        ...
        public virtual PasswordChangeResult ChangePassword(ChangePasswordRequest request)
        {
            ...
            //rainChange 当Email不知主键时，使用用户名做标识
            Customer customer = null;
            if (_customerSettings.UsernamesEnabled)
                customer = _customerService.GetCustomerByUsername(request.Email);
            else
                customer = _customerService.GetCustomerByEmail(request.Email);
