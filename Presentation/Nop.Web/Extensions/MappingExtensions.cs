    ...
    public static class MappingExtensions
    {
            ...
            Customer customer = null)
        {
            ...
            if (addressSettings.CountryEnabled && loadCountries != null)
            {
                ...
                model.AvailableCountries.Add(new SelectListItem() { Text = localizationService.GetResource("Address.SelectCountry"), Value = "-1" });
                ...
                if (addressSettings.StateProvinceEnabled)
                {
                    ...
                    var states = stateProvinceService.GetMyProvinces().ToList();
                    model.AvailableStates.Add(new SelectListItem() { Text = localizationService.GetResource("Address.Select"), Value = "-1" });
                    if (states.Count > 0)
                    {
                    ...
                    model.AvailableCities.Add(new SelectListItem() { Text = localizationService.GetResource("Address.Select"), Value = "-1" });
                    if (model.StateProvinceId > 0)
                    {
                        var cities = stateProvinceService.GetStateProvincesByParendId((int)model.StateProvinceId).ToList();
                        if (cities.Count > 0)
                        {
                    ...
                    model.AvailableDistricts.Add(new SelectListItem() { Text = localizationService.GetResource("Address.Select"), Value = "-1" });
                    if (model.CityId > 0)
                    {
                        var districts = stateProvinceService.GetStateProvincesByParendId((int)model.CityId).ToList();
        ...
        public static Address ToEntity(this AddressModel model, Address destination, bool trimFields = true)
        {
            ...
            destination.CountryId = model.CountryId > 0 ? model.CountryId : null;
            destination.StateProvinceId = model.StateProvinceId > 0 ? model.StateProvinceId : null;
            destination.CityId = model.CityId > 0 ? model.CityId : null;
            destination.DistrictId = model.DistrictId > 0 ? model.DistrictId : null;
