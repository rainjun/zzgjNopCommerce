...
using Nop.Services.Commissions;
using Nop.Admin.Models.Commissions;
...
namespace Nop.Admin.Controllers
{
    public partial class CommissionController : BaseAdminController
    {
        ...
        [NonAction]
        private void PrePareModelAreaInfo(OrderListModel model)
        {
            //states
        ...
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCommissions))
            ...
            //rainAdd 增加佣金的状态
            //Commission statuses
            model.AvailableCommissionStatuses = CommissionStatus.Pending.ToSelectList(false).ToList();
            model.AvailableCommissionStatuses.Insert(0, new SelectListItem() { Text = _localizationService.GetResource("Admin.Common.All"), Value = "0" });
            ...
            model.IsLoggedInAsDistributor = _workContext.CurrentCustomer.IsDistributor();

            PrePareModelAreaInfo(model);
        ...
        public ActionResult OrderList(DataSourceRequest command, OrderListModel model)
        {
            ...
            CommissionStatus? commissionStatus = model.CommissionStatusId > 0 ? (CommissionStatus?)(model.CommissionStatusId) : null;

            //rainAdd　增加分销商的区域限制
            int stateId = model.StateProvinceId;
            int cityId = model.CityId;
            int districtId = model.DistrictId;

            if (_workContext.CurrentCustomer.IsDistributor(false))
            {
                stateId = _workContext.CurrentCustomer.GetAttribute<int>(SystemCustomerAttributeNames.StateProvinceId);
                cityId = _workContext.CurrentCustomer.GetAttribute<int>(SystemCustomerAttributeNames.CityId);
                districtId = _workContext.CurrentCustomer.GetAttribute<int>(SystemCustomerAttributeNames.DistrictId);
            }

            //load orders
            var orders = _commissionService.SearchOrders(model.StoreId, model.VendorId, 0, 0, 0,
                startDateValue, endDateValue, orderStatus,
                paymentStatus, shippingStatus, commissionStatus, model.CustomerEmail, model.OrderGuid,
                command.Page - 1, command.PageSize, CommissionListType.Order, stateId, cityId, districtId);
        ...
        public ActionResult GoToOrderId(OrderListModel model)
        {
            ...
            if (order != null)
                return RedirectToAction("Edit", "Commission", new { id = order.Id });
            else
                return Order();
        }
        ...
        [HttpPost]
        public ActionResult NewList(DataSourceRequest command, OrderListModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCommissions))
                return AccessDeniedView();
            ...
            var orders = _commissionService.SearchOrders(model.StoreId, model.VendorId, 0, 0, 0,
                startDateValue, endDateValue, orderStatus,
                paymentStatus, shippingStatus, commissionStatus, model.CustomerEmail, model.OrderGuid,
                command.Page - 1, command.PageSize, CommissionListType.New, stateId, cityId, districtId);
            var gridModel = new DataSourceResult
            {
                Data = orders.Select(x =>
                {
        ...
        #region Apply Commission

        public ActionResult ApplyCommission()
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCommissions))
                return AccessDeniedView();
            ...
            if (customer.IsDistributor(false))
            {
                stateId = _workContext.CurrentCustomer.GetAttribute<int>(SystemCustomerAttributeNames.StateProvinceId);
                cityId = _workContext.CurrentCustomer.GetAttribute<int>(SystemCustomerAttributeNames.CityId);
                districtId = _workContext.CurrentCustomer.GetAttribute<int>(SystemCustomerAttributeNames.DistrictId);

                var orders = _commissionService.SearchOrders(0, 0, 0, 0, 0,
                           null, null, null,
                           null, null, null, null, null,
                           0, int.MaxValue, CommissionListType.Order, stateId, cityId, districtId);

                if (orders != null && orders.Count > 0)
                {
                    //新建Commission
                    //读取订单中的所有产品，取到CommissionRatio（比例）值*price,保留到CommissionItem  
                    commission = new Commission()
                    {
                        OrderGuid = Guid.NewGuid(),
                        CustomerId = customer.Id,
                        CommissionStatusId = (int)CommissionStatus.Processing,
                        AppliedDateUtc = DateTime.UtcNow,
                        CreatedOnUtc = DateTime.UtcNow
                    };
                    //insert commission
                    _commissionService.InsertCommission(commission);
                    ...
                    foreach (var order in orders)
                    {
                        ...
                        {
                            var productCommissionRatio = orderItem.Product.CommissionRatio * (decimal)0.01;
                            //save Commission item
                            var csItem = new CommissionItem()
                            {
                                Commission = commission,
                                OrderItemId = orderItem.Id,
                                UnitPrice = orderItem.UnitPriceExclTax,
                                Ratio = productCommissionRatio,
                                UnitCommission = productCommissionRatio * orderItem.UnitPriceExclTax,
                                TotalCommission = productCommissionRatio * orderItem.UnitPriceExclTax * orderItem.Quantity
                            };
                            commission.CommissionItems.Add(csItem);
                            _commissionService.UpdateCommission(commission);
                            commissionTotal += csItem.TotalCommission;
                        }
                        //设置order的CommissionStatusId
                        order.CommissionStatus = CommissionStatus.Processing;
                        order.CommissionId = commission.Id;
                        _orderService.UpdateOrder(order);

                        orderPriceTotal += order.OrderTotal;
                        commission.Orders.Add(order);
                    }
                    commission.OrdersTotal = orderPriceTotal;
                    commission.CommissionsTotal = commissionTotal;
                    _commissionService.UpdateCommission(commission);
                }
                SuccessNotification(_localizationService.GetResource("Admin.Commission.Applied"));
            }
        ...
        [HttpPost]
        public ActionResult HistoryList(DataSourceRequest command, CommissionListModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCommissions))
                return AccessDeniedView();
            ...
            //load orders
            var orders = _commissionService.SearchCommissions(customerId,
                startDateValue, endDateValue, orderStatus,
                paymentStatus, shippingStatus, commissionStatus, model.CustomerEmail, model.OrderGuid,
                command.Page - 1, command.PageSize, stateId, cityId, districtId);
            var gridModel = new DataSourceResult
            {
                Data = orders.Select(x =>
                {
                    return new CommissionModel()
                    {
                        ...
                        OrdersTotal = _priceFormatter.FormatPrice(x.OrdersTotal, true, false),
                        CommissionsTotal = _priceFormatter.FormatPrice(x.CommissionsTotal, true, false),
                        CommissionStatus = x.CommissionStatus.GetLocalizedEnum(_localizationService, _workContext),
                        CreatedOn = _dateTimeHelper.ConvertToUserTime(x.CreatedOnUtc, DateTimeKind.Utc)
                    };
                }),
                Total = orders.TotalCount
        ...
        public ActionResult CancelOrder(int id)
        {
            ...
            if (_workContext.CurrentVendor != null)
                return RedirectToAction("Edit", "Commission", new { id = id });
        ...
        public ActionResult PdfInvoice(int orderId)
        {
            ...
            //a vendor does not have access to this functionality
            if (_workContext.CurrentVendor != null)
                return RedirectToAction("Edit", "Commission", new { id = orderId });
        ...
        public ActionResult BestsellersReportList(DataSourceRequest command, BestsellersReportModel model)
        {
            ...
            var categoryids = _categoryService.GetChildCategoryIds(model.CategoryId);
