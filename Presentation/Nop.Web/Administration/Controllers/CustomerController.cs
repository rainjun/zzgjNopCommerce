...
namespace Nop.Admin.Controllers
{
    public partial class CustomerController : BaseAdminController
    {
        ...
        protected string ValidateCustomerRoles(IList<CustomerRole> customerRoles)
        {
            ...
            //rainAdd 分销商不能使其他角色
            bool isInDistributorsRole = customerRoles.FirstOrDefault(cr => cr.SystemName == SystemCustomerRoleNames.Distributors) != null;
            if (isInDistributorsRole)
            {
        ...
        public ActionResult Create()
        {
            ...
            PrePareCustomerModelAreaInfo(model);
        ...
        public ActionResult Create(CustomerModel model, bool continueEditing, FormCollection form)
        {
            ...
            //验证每个区域只有一个分销商
            ValidatorDistributorArea(model);
            ...
            if (ModelState.IsValid)
            {
                ...
                SaveCustomerAreaInfo(customer, model);
        ...
        /// <summary>
        /// rainAdd
        /// 为customerModel的地区信息做预处理
        /// </summary>
        /// <param name="model"></param>
        [NonAction]
        private void PrePareCustomerModelAreaInfo(CustomerModel model)
        {
            if (_customerSettings.CountryEnabled)
            {
                model.AvailableCountries.Add(new SelectListItem() { Text = _localizationService.GetResource("Admin.Address.SelectCountry"), Value = "-1" });
                ...
                {
                    //states
                    //rainChange 修改为获取上级ID为0的信息
                    var states = _stateProvinceService.GetMyProvinces().ToList();
                    model.AvailableStates.Add(new SelectListItem() { Text = _localizationService.GetResource("Address.Select"), Value = "-1" });
                    if (states.Count > 0)
                    {
                    ...
                    model.AvailableCities.Add(new SelectListItem() { Text = _localizationService.GetResource("Address.Select"), Value = "-1" });
                    if (model.StateProvinceId > 0)
                    {
                        var cities = _stateProvinceService.GetStateProvincesByParendId(model.StateProvinceId).ToList();
                        if (cities.Count > 0)
                        {
                    ...
                    model.AvailableDistricts.Add(new SelectListItem() { Text = _localizationService.GetResource("Address.Select"), Value = "-1" });
                    if (model.CityId > 0)
                    {
                        var districts = _stateProvinceService.GetStateProvincesByParendId(model.CityId).ToList();
                        if (districts.Count > 0)
                        {
                            foreach (var s in districts)
                                model.AvailableDistricts.Add(new SelectListItem() { Text = s.GetLocalized(x => x.Name), Value = s.Id.ToString(), Selected = (s.Id == model.DistrictId) });
                        }
                    }
                }
            }
        ...
        /// <param name="model"></param>
        [NonAction]
        private void ValidatorDistributorArea(CustomerModel model)
        {
            var distributorRole = _customerService.GetCustomerRoleBySystemName(SystemCustomerRoleNames.Distributors);
            bool isSelectedDistributor = true;
            if (model.SelectedCustomerRoleIds != null)
            {
                isSelectedDistributor = model.SelectedCustomerRoleIds.Contains(distributorRole.Id);
            }
            //假如是经销商，那么不允许地区重复
            if (isSelectedDistributor)
            {
                //获取当前区域位置
                int stateId = model.StateProvinceId > 0 ? model.StateProvinceId : 0;
                int cityId = model.CityId > 0 ? model.CityId : 0;
                int districtId = model.DistrictId > 0 ? model.DistrictId : 0;
                //获取所有经销商的信息，比对区域信息，不能重复
                var distributors = _customerService.GetAllCustomers(null, null, 0, 0, new[] { distributorRole.Id });
                if (distributors != null && distributors.Count > 0)
                {
                    string distributorError = string.Empty;
                    foreach (var distibutor in distributors)
                    {
                        if (distibutor.GetAttribute<int>(SystemCustomerAttributeNames.StateProvinceId) == stateId
                            && distibutor.GetAttribute<int>(SystemCustomerAttributeNames.CityId) == cityId
                            && distibutor.GetAttribute<int>(SystemCustomerAttributeNames.DistrictId) == districtId)
                        {
                            distributorError = "分销商区域设置不能重复";
                            break;
                        }
                    }
                    if (!string.IsNullOrEmpty(distributorError))
                    {
                        ModelState.AddModelError("", distributorError);
                    }
                }
            }
        }
        ...
        /// <param name="model"></param>
        [NonAction]
        private void SaveCustomerAreaInfo(Customer customer, CustomerModel model)
        {
            //Area
            ...
            if (_customerSettings.CountryEnabled && _customerSettings.StateProvinceEnabled && model.StateProvinceId > 0)
            {
                ...
                //rainAdd 增加选择了市、区，匹配地址名
                if (model.CityId > 0)
                {
                    _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.CityId, model.CityId);
                    if (model.DistrictId > 0)
                    {
                        _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.DistrictId, model.DistrictId);
                        Nop.Core.Domain.Directory.StateProvince tempArea = _stateProvinceService.GetStateProvinceById(model.CityId);
                        if (null != tempArea)
                        {
                            _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.City, tempArea.Name);
                        }
                        tempArea = _stateProvinceService.GetStateProvinceById(model.DistrictId);
                        if (null != tempArea)
                        {
                            _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.StreetAddress, tempArea.Name);
                        }
                    }
                }
            }
            //身份证
            //rainAdd 增加保存身份证号,格式正确则一并取生日信息
            if (!string.IsNullOrWhiteSpace(model.IdentityId))
            {
                DateTime? tempDatetime = DateTime.UtcNow;
                if (CommonHelper.IsValidIdCardNo(model.IdentityId, out tempDatetime))
                {
                    customer.IdentityId = model.IdentityId;
                    _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.DateOfBirth, tempDatetime);
                }
            }
            else if (_customerSettings.DateOfBirthEnabled)
                _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.DateOfBirth, model.DateOfBirth);
        }
        ...
        public ActionResult Edit(int id)
        {
            ...
            model.DistrictId = customer.GetAttribute<int>(SystemCustomerAttributeNames.DistrictId);
        ...
        public ActionResult ChangePassword(CustomerModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                var changePassRequest = new ChangePasswordRequest(_customerSettings.UsernamesEnabled ? model.Username : model.Email,
        ...
        public ActionResult AddressCreate(int customerId)
        {
            ...
            //countries
            model.Address.AvailableCountries.Add(new SelectListItem() { Text = _localizationService.GetResource("Admin.Address.SelectCountry"), Value = "-1" });
            foreach (var c in _countryService.GetAllCountries(true))
                model.Address.AvailableCountries.Add(new SelectListItem() { Text = c.Name, Value = c.Id.ToString() });
            model.Address.AvailableStates.Add(new SelectListItem() { Text = _localizationService.GetResource("Admin.Address.Select"), Value = "-1" });
            model.Address.AvailableCities.Add(new SelectListItem() { Text = _localizationService.GetResource("Admin.Address.Select"), Value = "-1" });
            model.Address.AvailableDistricts.Add(new SelectListItem() { Text = _localizationService.GetResource("Admin.Address.Select"), Value = "-1" });
        ...
        /// <param name="model"></param>
        [NonAction]
        protected void PrepareAddressModel(AddressModel model)
        {
            model.CountryId = model.CountryId > 0 ? model.CountryId : null;
            model.StateProvinceId = model.StateProvinceId > 0 ? model.StateProvinceId : null;
            model.CityId = model.CityId > 0 ? model.CityId : null;
            model.DistrictId = model.DistrictId > 0 ? model.DistrictId : null;
            if (model.CityId > 0)
            {
                Nop.Core.Domain.Directory.StateProvince tempArea = _stateProvinceService.GetStateProvinceById((int)model.CityId);
                model.City = tempArea.Name;
                if (model.DistrictId > 0)
                {
                    tempArea = _stateProvinceService.GetStateProvinceById((int)model.DistrictId);
                    model.Address1 = tempArea.Name;
                }
            }
        }
        ...
        public ActionResult AddressCreate(CustomerAddressModel model)
        {
            ...
            PrePareAddressModelAreaInfo(model.Address);
        ...
        /// <param name="model"></param>
        [NonAction]
        private void PrePareAddressModelAreaInfo(AddressModel model)
        {
            //countries
            model.AvailableCountries.Add(new SelectListItem() { Text = _localizationService.GetResource("Admin.Address.SelectCountry"), Value = "-1" });
            foreach (var c in _countryService.GetAllCountries(true))
            ...
            //states
            //rainChange 增加对市区的适配
            model.AvailableStates.Add(new SelectListItem() { Text = _localizationService.GetResource("Admin.Address.Select"), Value = "-1" });
            var states = model.CountryId > 0 ? _stateProvinceService.GetMyProvinces().ToList() : new List<StateProvince>();
            if (states.Count > 0)
            {
                foreach (var s in states)
            ...
            model.AvailableCities.Add(new SelectListItem() { Text = _localizationService.GetResource("Admin.Address.Select"), Value = "-1" });
            var cities = model.StateProvinceId > 0 ? _stateProvinceService.GetStateProvincesByParendId(model.StateProvinceId.Value).ToList() : new List<StateProvince>();
            if (cities.Count > 0)
            {
                foreach (var s in cities)
            ...
            model.AvailableDistricts.Add(new SelectListItem() { Text = _localizationService.GetResource("Admin.Address.Select"), Value = "-1" });
            var districts = model.CityId > 0 ? _stateProvinceService.GetStateProvincesByParendId(model.CityId.Value).ToList() : new List<StateProvince>();
            if (districts.Count > 0)
            {
                foreach (var s in districts)
                    model.AvailableDistricts.Add(new SelectListItem() { Text = s.Name, Value = s.Id.ToString(), Selected = (s.Id == model.DistrictId) });
            }
        }
