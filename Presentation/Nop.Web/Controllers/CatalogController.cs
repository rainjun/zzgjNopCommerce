...
namespace Nop.Web.Controllers
{
    public partial class CatalogController : BasePublicController
    {
        ...
        /// 根据类型输出产品
        /// </summary>
        /// <param name="type">默认、new最新,hot,priceasc,pricedesc</param>
        /// <param name="catalogid">默认0全部</param>
        /// <param name="slug">catalog的slug</param>
        /// <param name="incolusub">默认true</param>
        /// <param name="productThumbPictureSize">图片尺寸，默认null</param>
        /// <param name="count">默认3个</param>
        /// <param name="partviewname">自定义页面的后缀</param>
        /// <returns></returns>
        [NopHttpsRequirement(SslRequirement.No)]
        [ChildActionOnly]
        public ActionResult HomePageProductsByType(string type = "", int catalogid = 0, string slug = "", bool incolusub = true, int? productThumbPictureSize = null, int count = 0, string partviewname = "")
        {
            var model = new List<ProductOverviewModel>();
            List<int> catalogIds = null;
            #region get by slug
            if (catalogid <= 0 &&
                !string.IsNullOrEmpty(slug))
            {
                //rainAdd根据seo的slug查询对应的类别信息
                var urlRecordService = EngineContext.Current.Resolve<IUrlRecordService>();
                var urlRecord = urlRecordService.GetBySlug(slug);
                if (urlRecord != null)
                {
                    catalogid = urlRecord.EntityId;
                }
            }
            #endregion

            #region get from subcategories
            if (catalogid > 0)
            {
                catalogIds = GetChildCategoryIds(catalogid);
            }
            #endregion
            ...
            else if ("hot".Equals(type.ToLower()))
            {
                #region hot
                //load and cache report
                var products = _cacheManager.Get(cacheKey,
            ...
            else if ("priceasc".Equals(type.ToLower()) ||
                "pricedesc".Equals(type.ToLower()))
            {
                #region price
                ProductSortingEnum po = ProductSortingEnum.PriceAsc;
                if ("pricedesc".Equals(type.ToLower()))
                {
            ...
            string viewName = "HomePageProductsByType" + type + partviewname;
        ...
        #region new Searching

        //public ActionResult MySearch(CatalogPagingFilteringModel command)
        //{
        //    //'Continue shopping' URL
        //    _genericAttributeService.SaveAttribute(_workContext.CurrentCustomer,
        //        SystemCustomerAttributeNames.LastContinueShoppingPage,
